...
//					pieces.push_back(piece);
					...
					calPiece(piece, coff_sum, integer_sum, ops);

					string ops_str{equation[j]}; 
					if(equation[j] == '='){
						ops_str = "-";
						bitwise = 1;
					}
...
//					pieces.push_back(ops_str);
					calPiece(ops_str, coff_sum, integer_sum, ops);
...
//		for(auto &piece : pieces)
	...
	int calPiece(string &piece, int &coff_sum, int &integer_sum, char &ops)
	{
		{
			if(*(piece.rbegin()) == 'x')
			{
